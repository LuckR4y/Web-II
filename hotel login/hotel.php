...
if (isset($_POST['cadastrar'])) {
    ...
    $aposento = $_POST['aposento'];

    // Calcular a diferença de dias entre dataEntrada e dataSaida
    $entrada = new DateTime($dataEntrada);
    $saida = new DateTime($dataSaida);
    $intervalo = $entrada->diff($saida);
    $diasHospedagem = $intervalo->days;

    // Obter o valor da diária do quarto selecionado
    $res = $mysqli->query("SELECT valor FROM aposentos WHERE id = $aposento");
    $quarto = $res->fetch_assoc();
    $valorDiaria = $quarto['valor'];

    // Calcular o total da hospedagem
    $valorTotal = $diasHospedagem * $valorDiaria;

    // Inserir hóspede no banco de dados
    $stmt = $mysqli->prepare("INSERT INTO hospedes (nome, cpf, rg, telefone) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $nome, $cpf, $rg, $telefone);
    $stmt->execute();
    $hospede_id = $stmt->insert_id;

    // Criar uma nova conta com o valor total da hospedagem
    $mysqli->query("INSERT INTO contas (valorTotal, pago) VALUES ($valorTotal, 0)");
    $conta_id = $mysqli->insert_id;

    // Atualizar o quarto para "ocupado"
    $mysqli->query("UPDATE aposentos SET ocupado = 1 WHERE id = $aposento");

    // Registrar a hospedagem
    $stmt = $mysqli->prepare("INSERT INTO hospedagens (dataEntrada, dataSaida, hospede_id, aposento_id, conta_id) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("ssiii", $dataEntrada, $dataSaida, $hospede_id, $aposento, $conta_id);
    $stmt->execute();

    echo "<p>Hóspede cadastrado com sucesso! Valor total da diária: R$ $valorTotal</p>";
}
